...
状态：本地 Node.js 24 测试和服务启动验证通过；远程宝塔面板尚未部署。根据用户要求，本步使用 Node.js，不使用 Compose。当前服务是 ELSDTU 的最小验证程序，**尚未接入 ThingsBoard，也不是完整生产中台**。
...
本地验证记录：使用桌面应用附带的 Node.js v24.19.0 直接运行 `node --test`，1 项集成测试通过。随后直接运行 `src/server.mjs`，实际 HTTP 请求得到健康检查 `ok`、网关心跳前 `online=false`、心跳后 `online=true`、未授权请求 401；重启服务后网关记录仍存在。系统默认的 Node.js v22.15.0 在当前 Codex Windows 沙箱中解析项目路径时返回 `EPERM`，因此本次直接运行验证使用 Node.js 24。远程运行仍需在宝塔面板验收。
